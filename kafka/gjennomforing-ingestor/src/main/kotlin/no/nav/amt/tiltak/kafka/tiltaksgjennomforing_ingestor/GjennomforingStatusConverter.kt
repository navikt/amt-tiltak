...
object GjennomforingStatusConverter {
	...
	private val ikkeStartetStatuser = listOf(GjennomforingMessage.Status.APENT_FOR_INNSOK.name, GjennomforingMessage.Status.PLANLAGT.name)
	...
	fun convert (status: String) : Gjennomforing.Status {
		return when (status) {
			...
			in ikkeStartetStatuser -> Gjennomforing.Status.PLANLAGT
