...
@Service
class MineDeltakerlisterServiceImpl(
	...
	override fun leggTil(
		...
		val harAlleredeTilgang =
			hent(arrangorAnsattId)
				.contains(gjennomforingId)

		if (harAlleredeTilgang) {
			log.info("Kan ikke opprette tilgang på gjennomføring $id siden arrangør ansatt $arrangorAnsattId allerede har tilgang")
			return
		}

		if (gjennomforingService.getGjennomforingOrNull(gjennomforingId) == null) {
			log.info("Kan ikke opprette tilgang på gjennomføring $id for ansatt $arrangorAnsattId fordi gjennomføringen ikke finnes")
			return
		}
	...
	override fun fjernAlleHosArrangor(
		arrangorAnsattId: UUID,
		arrangorId: UUID,
	) {
		val tilganger =
			mineDeltakerlisterRepository
				.hent(arrangorAnsattId)
