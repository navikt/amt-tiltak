...
class KafkaMessageSender(
	...
	@Value("\${app.env.amtTiltakTopic}") private val amtTiltakTopic: String,
	@Value("\${app.env.sisteTiltaksgjennomforingerTopic}") private val sisteTiltaksgjennomforingerTopic: String,
	@Value("\${app.env.skjermedePersonerTopic}") private val skjermedePersonerTopic: String,
	@Value("\${app.env.leesahTopic}") private val leesahTopic: String,
	@Value("\${app.env.aktorV2Topic}") private val aktorV2Topic: String,
	@Value("\${app.env.amtArrangorTopic}") private val amtArrangorTopic: String,
) {
	...
	fun sendTilSkjermetPersonTopic(fnr: String, erSkjermet: Boolean) {
		kafkaProducer.send(
			ProducerRecord(
				...
				fnr.toByteArray(),
				erSkjermet.toString().toByteArray()
			)
		)
	}
